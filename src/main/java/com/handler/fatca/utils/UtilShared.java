package com.handler.fatca.utils;
...
public class UtilShared {
	...
	public static boolean validateSchema(String xmlFile, String schemaFile, String startElem) throws Exception {
		...
		try {
			...
			if (schemaFile.startsWith("http"))
				...
				.newValidator().validate(new StAXSource(reader));
			success = true;
			log.debug("vaLidation ended. time=" + new Date());
			reader.close();
			br.close();
			reader = null;
			br = null;
    ...
    public static void printNode(Node node) {
		log.debug("--> printNode() " + node.getNodeName());
		log.debug("prefix=" + node.getPrefix() + ", baseuri=" + node.getBaseURI() + ", nsuri=" + node.getNamespaceURI() + ", value=" + node.getNodeValue());
		if (node.getFirstChild() != null) {
			log.debug("--> Child of " + node.getNodeName() );
			printNode(node.getFirstChild());
            log.debug("<-- Child of " + node.getNodeName() );
		}
		if (node.getNextSibling() != null) {
            log.debug("--> Sibling of " + node.getNodeName() );
	...
	public static void createBinaryFileFromSignedBase64BinaryFile(String infile, String outfile) throws Exception {
        log.debug("--> createBinaryFileFromSignedBase64BinaryFile(). infile=" + infile + ", outfile=" + outfile);
		BufferedReader br = null;
        ...
        log.debug("<-- createBinaryFileFromSignedBase64BinaryFile()");
	}
	...
	public static void extractUnsignedXmlFromSignedXml(String signedXml, String[] unsignedXmlStartElem, String[] outUnsignedXml) throws Exception {
        log.debug("--> extractUnsignedXmlFromSignedXml(). signedXml=" + signedXml);
		if (signedXml == null)
			...
			while (!finished && rd.hasNext()) {
				...
				case XMLStreamConstants.START_ELEMENT:
					...
					//start looking for start element from next element ('else' skips referenced element)
						...
						log.debug("start extracting. refId=" + curRefId + ", startTag=" + unsignedXmlStartElem[curIndex]);
					}
				...
				case XMLStreamConstants.END_ELEMENT:
					if (startElemFound) {
						...
						if (stackUnsignedElemTag.isEmpty()) {
							log.debug("end extracting. refId=" + curRefId + ", startTag=" + unsignedXmlStartElem[curIndex]);							bw.close();
							bw = null;
		...
		log.debug("<-- extractUnsignedXmlFromSignedXml()");
	}
}
