...
public class SignatureVerifier implements ISignatureVerifier {
	...
	protected class SignatureInfo {
		...
		protected void setSignedInfoFromXml(String xml, DocumentBuilder docBuilderNSTrue) throws Exception {
			log.trace("--> setSignedInfoFromXml(). xml=" + xml);
			...
			log.trace("<-- setSignedInfoFromXml()");
		}
		...
		protected boolean isCalcDigestSameAsDocDigest() {
			...
			for (int i = 0; i < signedInfo.references.size(); i++) {
				...
				if (ref.calcDigestVal == null || ref.digestVal == null)
					return false;
				log.debug("referenceURI=" + ref.uri);
				log.debug("calcDigest  =" + ref.calcDigestVal);
				log.debug("docDigest   =" + ref.digestVal);
	...
	protected class DynRefInfo {
		...
		SignatureInfo.ReferenceItem ref = null;
		QName qname = null;
	...
	protected class KeyValueKeySelector extends KeySelector {
		public KeySelectorResult select(KeyInfo keyInfo, Purpose purpose,
			...
			if (pk != null) {
				...
				log.debug("PublicKey from XML=" + pk);
	...
	protected DynRefInfo getStartElem(XMLStreamReader reader, StringBuilder sbElem, XmlTag lastStartTag,
		...
		if (count > 0) {
			...
			for (int i = 0; i < count; i++) {
				...
				if ("Id".equals(localname) && sigInfo != null && sigInfo.signedInfo != null && qname.getNamespaceURI().equals(sigInfo.qname.getNamespaceURI())) {
					...
					SignatureInfo.ReferenceItem ref = sigInfo.getReference(tmpS);
					if (ref != null) {
		...
		sbElem.append(">");
		log.trace("sbElem=" + sbElem.toString());
		log.trace("<-- getStartElem()");
		return dynRefInfo;
	}
	...
	protected void updateDigest(StringBuilder parseBuf, DynRefInfo dynRefInfo, DocumentBuilder docBuilderNSTrue) throws Exception {
		log.trace("--> updateDigest(). dynRefInfo=" + dynRefInfo.toString());
		try {
			...
			log.debug("parseBuf=" + parseBuf.toString());
			log.error(e.getMessage());
			throw e;
		}
		log.trace("<-- updateDigest()");
	}
	...
	protected void updateDigestWithXmlChunk(StringBuilder parseBuf, DynRefInfo dynRefInfo, DocumentBuilder docBuilderNSTrue) throws Exception {
		log.trace("--> updateDigestWithXmlChunk(). dynRefInfo=" + dynRefInfo);
		try {
			log.trace("dynRefInfo=" + dynRefInfo);
	    	...
	    	log.trace("parseBuf=" + parseBuf.toString());
	    	log.trace("to transform str startPrefixTags=" + startPrefixTags);
	    	log.trace("to transform str endSuffixTags  =" + endSuffixTags);
	    	String modifiedval = startPrefixTags + parseBuf.toString() + endSuffixTags;
	    	log.trace("modifiedval    =" + modifiedval);
			...
			log.debug("parseBuf=" + parseBuf.toString());
			log.debug("dynRefInfo=" + dynRefInfo.toString());
			log.error(e.getMessage());
			throw e;
		}
		log.trace("<-- updateDigestWithXmlChunk()");
	}
	...
	protected boolean verifySignature(String signedFile, PublicKey sigkey, boolean useFakeElem) throws Exception  {
		log.debug("--> verifySignature(). signedFile=" + signedFile + ", sigkey=" + sigkey + ", useFakeElem=" + useFakeElem);
		boolean ret = false;
		...
		try {
			...
			log.trace("isFakeElem=" + isFakeElem);
			if (isFakeElem) {
			...
			log.debug("Signature core verification=" + coreVerification);
		    SignatureValue sigval = signature.getSignatureValue();
		    sigValVerification = sigval.validate(valContext);
		    log.debug("SignatureValue varification status=" + sigValVerification);
	        ...
	        for (int j=0; iter.hasNext(); j++) {
	        	...
	        	log.debug("ref["+j+"] verification status=" + refVerification);
	        	refVerificationFlag &= refVerification;
	        	log.debug("ref.getURI()=" + ref.getURI());
	        	log.debug("ref.getCalculatedDigestValue()=" + Base64.encode(ref.getCalculatedDigestValue()));
	        	log.debug("ref.getReferencedDigestValue()=" + Base64.encode(ref.getDigestValue()));
	        	List<?> lt = ref.getTransforms();
	        	Transform tr;
	        	for (int i = 0; i < lt.size(); i++) {
	        		tr = (Transform)lt.get(i);
	        		log.debug("transform.getAlgorithm()=" + tr.getAlgorithm());
	        	}
	        }
		...
		log.debug("signature verification=" + ret);
	...
	public boolean verifySignatureStreaming(String signedXmlFile, PublicKey sigPublicKey) throws Exception {
		log.debug("--> verifySignatureStreaming(). signedXmlFile=" + signedXmlFile + ", sigPublicKey=" + sigPublicKey);
		...
		try {
			...
			while(!finished) {
				...
				case XMLStreamConstants.END_ELEMENT:
				    ...
				    if (dynRefInfo != null && dynRefInfo.stackStartTag.empty()) {
				    	...
				    	log.trace("calcDigestVal=" + dynRefInfo.ref.calcDigestVal);
				    	log.trace("docDigestVal =" + dynRefInfo.ref.digestVal);
				    	if (dynRefInfo.ref.digestVal.equals(dynRefInfo.ref.calcDigestVal))
				    		log.trace("Same Digest");
				    	dynRefInfo = null;
			...
			log.debug("digestVerificationFlag=" + digestVerificationFlag);
			boolean sigValueVerificationFlag = sigInfo.verifySignature();
			log.debug("sigValueVerificationFlag=" + sigValueVerificationFlag);
			success = digestVerificationFlag & sigValueVerificationFlag;
		} catch(Exception e) {
			e.printStackTrace();
			log.error("infile=" + signedXmlFile + ", exception msg=" + e.getMessage());
			throw e;
		} finally {
		...
		log.debug("<-- verifySignatureStreaming()");
