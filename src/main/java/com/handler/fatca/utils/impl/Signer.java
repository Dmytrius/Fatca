package com.handler.fatca.utils.impl;
...
import com.handler.fatca.utils.UtilShared.XmlTag;
...
public class Signer implements ISigner {
    ...
    protected void writeBase64BinaryAndOptionallyCalcMsgDigest(String infile, String newline, Writer writer,
		...
		log.debug("--> writeBase64BinaryAndOptionallyCalcMsgDigest(). infile=" + infile);
		...
		log.debug("<-- writeBase64BinaryAndOptionallyCalcMsgDigest()");
	}
	...
	protected void updateDigestWithXmlChunk(StringBuilder parseBuf, MessageDigest messageDigest, Stack<XmlTag> stackStartTag,
		...
		log.trace("to transform str=" + modifiedval);
		...
		log.trace("transformed str=" + digestval);
	...
	protected void calcCanonicalizedXmlMsgDigestByParsingDocChunk(Writer writer, SignedDocument.ReferenceInfo ri,
			SigXmlTransform sigXmlTransform) throws Exception {
		log.debug("--> calcCanonicalizedXmlMsgDigestByParsingDocChunk(). infile=" + ri.infile + ", sigXmlTransform=" + sigXmlTransform);
		...
		try {
			...
			e.printStackTrace();
			log.error("infile=" + ri.infile + ", exception msg=" + e.getMessage());
			throw e;
		} finally {
			if (reader != null) try{reader.close();}catch(Throwable t){}
			if (fis != null) try{fis.close();}catch(Throwable t){}
		}
		log.debug("<-- calcCanonicalizedXmlMsgDigestByParsingDocChunk()");
	}

    //calculate message digest as is - no transformation. used to sign wrapped base64 binary and wrapped text document
	protected void calcXmlMsgDigestNoTransformation(SignedDocument.ReferenceInfo ri) throws Exception {
		log.debug("--> calcXmlMsgDigestNoTransformation(). infile=" + ri.infile);
		int len;
		...
		log.debug("<-- calcXmlMsgDigestNoTransformation()");
    }

	//not used in this packaging. this calculate message digest of a text file - no transformation
	protected void calcTextMsgDigestNoTransformation(SignedDocument.ReferenceInfo ri) throws Exception {
		log.debug("--> calcTextMsgDigestNoTransformation(). infile=" + ri.infile);
		int len;
		String tmp;
		String[] tmparr = ri.getReferencePrefixAndSuffix();
		String sigRefIdPrefix = tmparr[0];
		...
		log.debug("<-- calcTextMsgDigestNoTransformation()");
    }
	...
	protected SignedDocument createSignedDOMDoc(String[] infiles, PrivateKey sigkey, X509Certificate sigPubCert,
		...
		log.debug("--> createSignedDOMDoc(). infiles=" + prettyNames(infiles) + ", sigDocType=" + sigDocType + ", sigXmlTransform=" + sigXmlTransform);
        ...
        Document doc = null;
        Node[] nodes = null;
    	SignedDocument.ReferenceInfo ri = null;
	...
	protected boolean writeTextFileObjectContent(String infile, Writer writer) throws Exception {
		log.debug("--> writeTextFileObjectContent(). infile=" + infile);
    	boolean ret = false;
    	String tmp;
		...
		log.debug("<-- writeTextFileObjectContent()");
    	return ret;
	}
    ...
    protected boolean signFileStreaming(String[] infiles, String outfile, PrivateKey sigkey, X509Certificate sigPubCert,
		...
		log.debug("--> signFileStreaming(). infiles=" + prettyNames(infiles) + ", outfile=" + outfile + ", sigDocType=" + sigDocType + ", sigXmlTransform=" + sigXmlTransform);
    	boolean ret = false;
    	...
    	try {
            ...
            if (nodeList.getLength() > 0) {
                ...
                log.debug("SignedInfo=" + new String(signedInfoBuf));
            ...
            if (sigDocType == SigDocType.XML) {
    	        ...
    	        if (tmps.length() != deststr.length()) {
    	        	log.error("blank signed doc upto tmps len=" + tmps.length() + ", tmps=" + tmps);
    	        	log.error("deststr after SignatureValue and DigestValue(s) modified len=" + deststr.length() + ", deststr=" + deststr);
    ...
    protected boolean signFile(String[] infiles, String outfile, PrivateKey sigkey, X509Certificate sigPubCert,
    		SigDocType sigDocType, SigXmlTransform sigXmlTransform) throws Exception {
		log.debug("--> signFile(). infiles=" + prettyNames(infiles) + ", outfile=" + outfile + ", sigDocType=" + sigDocType + ", sigXmlTransform=" + sigXmlTransform);
		BufferedWriter bw = null;
		boolean ret = false;
    	try {
	        ...
	        if (nodeList.getLength() > 0)
	        	log.debug("DigestValue=" + nodeList.item(0).getFirstChild().getNodeValue());
	    	bw = new BufferedWriter(new FileWriter(new File(outfile)));
    ...
    protected boolean wrapBinaryFileInXmlAndSign(String[] infiles, String outfile, PrivateKey sigkey,
		...
		log.debug("--> wrapBinaryFileInXmlAndSign(). infiles=" + prettyNames(infiles) + ", outfile=" + outfile + ", isDOM=" + isDOM);
		boolean flag = false;
		...
		log.debug("<-- wrapBinaryFileInXmlAndSign()");
		return flag;
    }
    ...
    protected boolean wrapTextFileInXmlAndSign(String[] infiles, String outfile, PrivateKey sigkey,
    		X509Certificate sigPubCert, boolean isDOM) throws Exception {
		log.debug("--> wrapTextFileInXmlAndSign(). infiles=" + prettyNames(infiles) + ", outfile=" + outfile + ", isDOM=" + isDOM);
		boolean flag = false;
    	int len;
        char[] tmpBuf = new char[UtilShared.defaultBufSize];
		String[] tags = getWrapperTags();
		...
		log.debug("<-- wrapTextFileInXmlAndSign()");
		return flag;
    }
    ...
    protected boolean signFileNoWrap(String[] infiles, String outfile, PrivateKey sigkey, X509Certificate sugPubCert,
		...
		log.debug("--> signFileNoWrap(). infiles=" + prettyNames(infiles) + ", outfile=" + outfile + ", isDom=" + isDom + ", sigDocType=" + sigDocType + ", sigXmlTransform=" + sigXmlTransform);
    	boolean flag = false;
    	if (isDom)
