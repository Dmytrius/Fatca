package com.handler.fatca.utils.main;
...
public class FATCADataPrepTool {
	...
	protected X509Certificate receiverPublicCert = null;
	protected X509Certificate senderPublicCert = null;
    protected X509Certificate approverPublicCert = null;
	protected PrivateKey receiverPrivateKey = null;
    protected PrivateKey senderPrivateKey = null;
    protected PrivateKey approverPrivateKey = null;
	protected String senderGiin = null;
    protected String receiverGiin = null;
    protected String approverGiin = null;
    protected String xmlSchema = null;
    protected String xmlStartElemForSchemaValidation = null;
	protected int taxyear = -1;
	boolean isVerifyAllSignature = false;
	boolean isValidateAllXMLSchema = false;
    ...
    protected void readConfigAndExceuteCommands(String conf) throws Exception {
        ...
        try {
            ArrayList<Cmd> listCmds = new ArrayList<>();
            ...
            if (isVerifyAllSignature && signatureVerifier != null)
                log.info("all signature verification=" + signatureVerifier.getVerificationFlag());
        } catch(Exception e) {
            e.printStackTrace();
            throw e;
        }
	...
	protected void processCmds(ArrayList<Cmd> listCmds) throws Exception {
		...
		for (int i = 0; i < listCmds.size(); i++) {
			...
			if ("pause".equalsIgnoreCase(cmd.cmdStr)) {
				...
				log.info(signer.getProperty("getSigningDebugBuf").toString());
				continue;
			...
			if ("signBinary".equalsIgnoreCase(cmd.cmdStr) || 
				...
				log.error("ERROR: unable to execute " + cmd.cmdStr + "....there may be missing info to execute cmd");
			log.info("finished...cmd=" + cmd.cmdStr + (input==null?"":" input=" + input) + (output==null?"":", output=" + output) + " [" + cmd.cmdLine + "]");
		}
	}
	...
	protected void processExtractUnsignedXmlFromSignedXml(Cmd cmd) throws Exception {
		...
		if (signedXml == null || unsignedXmlStartElem == null || outUnsignedXml == null)
			log.error("ERROR: unable to execute " + cmd.cmdStr + "....signedXml == null || unsignedXmlStartElem == null || outUnsignedXml == null ...there may be missing info,  to execute cmd");
		else
	...
	protected void processSignatureCmds(String input, String output, Cmd cmd) throws Exception {
		...
		if (isValidateAllXMLSchema) {
			if (xmlSchema != null && xmlStartElemForSchemaValidation != null) {
				...
				log.info("schema validation=" + flag + ", xml=" + input + ", schema=" + xmlSchema + 
						", xmlStartElemForSchemaValidation=" + xmlStartElemForSchemaValidation);
			} else {
				log.info("schema wasn't validated. xml=" + input + ", schema=" + xmlSchema + 
						", xmlStartElemForSchemaValidation=" + xmlStartElemForSchemaValidation);
			}
		}
	...
	protected void processValidateSchemaCmd(Cmd cmd) throws Exception {
		...
		if (xmlFile == null || schemaFile == null)
			log.error("xmlFile or schemaFile is null [" + cmd.cmdLine + "]");
		else {
			boolean flag = UtilShared.validateSchema(xmlFile, schemaFile, startElem);
			log.info("schema validation=" + flag);
		}
	}
	...
	protected void processVerifySignatureCmd(String input, Cmd cmd) throws Exception {
		...
		log.info("signature verification=" + flag);
	}
